...
#script from https://github.com/AntonMu/TrainYourOwnYOLO
#personal contribution: image_crop_growth and image_crop_optimize()

import image_crop_growth

def image_crop_optimize(img_path, output_image, prediction, increment, background_colour, save_image, save_img_path, postfix=""): #255 for white
    ...
    for single_prediction in prediction:
        ...
        growth_limit_prediction = image_crop_growth.get_growth_limit(img_path, single_prediction, increment, background_colour)
        
        xmin, ymin, xmax, ymax = growth_limit_prediction[0], growth_limit_prediction[1], growth_limit_prediction[2], growth_limit_prediction[3]
        start_point = (xmin, ymin)
        end_point = (xmax, ymax)
        color = (255, 0, 0)
        rec_image = cv2.rectangle(output_image, start_point, end_point, color, thickness=2)
        #image_draw.rectangle([xmin, ymin, xmax, ymax], outline="blue")
        
        new_prediction.append(growth_limit_prediction)
        
    #img_out = postfix.join(os.path.splitext(os.path.basename(img_path)))
    filename = os.path.split(img_path)[1]
    save_filepath = os.path.join(save_img_path, filename)
    if save_image:
        #image_draw.save(os.path.join(save_img_path, img_out))
        cv2.imwrite(save_filepath, rec_image)
...
#edit #crop results folder
detection_results_crop_folder = os.path.join(image_folder, "Test_Image_Detection_Results_crop")
...
"""
def recursive_crop(img_path, prediction):
    #optimized prediction
    optimised_prediction = []
    #load image
    image = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE())
    image_thresh = cv2.threshold(image, 120, 255, cv2.THRESH_BINARY)
    ...
    for i, single_prediction in enumerate(prediction):
        ...
        #extract coordinates
        xmin_initial = single_prediction[0]
        ymin_initial = single_prediction[1]
        xmax_initial = single_prediction[2]
        ymax_initial = single_prediction[3]
"""
...
if __name__ == "__main__":
    ...
    if input_image_paths and not webcam_active:
        ...
        for i, img_path in enumerate(input_image_paths):
            ...
            new_prediction = image_crop_optimize(
                img_path,
                output_image=image,
            ...
            for single_prediction, single_new_prediction in zip(prediction, new_prediction):
